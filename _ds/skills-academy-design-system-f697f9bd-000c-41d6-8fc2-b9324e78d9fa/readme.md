...
**Animation / hover / press.** Not specified in either source document — no transitions, easings, or interaction states are defined. Treat as an open gap (flag to the user before inventing motion language).
...
## Iconography
No icon system, icon font, SVG set, or imagery of any kind was supplied in either source document. The only graphic asset is the logo mark itself (two intersecting ribbon strokes + one coral "spark" circle — see Brand cards). No emoji or Unicode-glyph icon usage observed. **If a screen needs icons, this system has no house style to match** — pick a neutral, rounded-stroke CDN set (e.g. Phosphor or Lucide, regular weight) as a placeholder and flag the substitution; do not hand-draw icons.

## Logo
The only real logo asset is the inline SVG mark from the source HTML — copied as-is into `assets/` (color, white/reverse, monochrome). It is explicitly marked **not final** in the source ("wymaga jeszcze finalnej wektoryzacji i pełnego zestawu plików produkcyjnych") — treat it as a placeholder mark pending a designer's final vector file, not production-ready artwork. Clear space = one mark-height on all sides. Minimum size 24px screen / 10mm print (drop the spark below that). Never rotate, mirror, or distort.

## Fonts
Lexend and Fredoka are loaded via a Google Fonts CDN `@import` in `tokens/fonts.css` (both are real, freely-licensed Google Fonts already used in the source mockup — no substitution needed). This sandbox could not fetch and vendor the actual `.woff2`/`.ttf` binaries, so fonts currently load over the network rather than being self-hosted. **If you want self-hosted font files for production**, download Lexend and Fredoka from Google Fonts and drop the files into `assets/fonts/`, and I'll switch `tokens/fonts.css` to local `@font-face` rules.
